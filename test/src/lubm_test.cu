...
void lubm(char *data_path) {
    ...
    timer.start_timer();
    // deduplicate
    src_advisor.deduplicate();
    src_AssistantProfessor.deduplicate();
    src_AssociateProfessor.deduplicate();
    src_Course.deduplicate();
    src_Department.deduplicate();
    src_doctoralDegreeFrom.deduplicate();
    src_emailAddress.deduplicate();
    src_FullProfessor.deduplicate();
    src_GraduateCourse.deduplicate();
    src_GraduateStudent.deduplicate();
    src_headOf.deduplicate();
    src_Lecturer.deduplicate();
    src_mastersDegreeFrom.deduplicate();
    src_memberOf.deduplicate();
    src_name.deduplicate();
    src_Publication.deduplicate();
    src_publicationAuthor.deduplicate();
    src_ResearchAssistant.deduplicate();
    src_ResearchGroup.deduplicate();
    src_researchInterest.deduplicate();
    src_subOrganizationOf.deduplicate();
    src_takesCourse.deduplicate();
    src_teacherOf.deduplicate();
    src_TeachingAssistant.deduplicate();
    src_teachingAssistantOf.deduplicate();
    src_telephone.deduplicate();
    src_undergraduateDegreeFrom.deduplicate();
    src_UndergraduateStudent.deduplicate();
    src_University.deduplicate();
    src_worksFor.deduplicate();
    timer.stop_timer();
    ...
    AssistantProfessor.total_tuples +=
        src_AssistantProfessor.get_versioned_size(NEWT);
    AssistantProfessor.deduplicate();
    ...
    AssociateProfessor.total_tuples +=
        src_AssociateProfessor.get_versioned_size(NEWT);
    AssociateProfessor.deduplicate();
    ...
    doctoralDegreeFrom.total_tuples +=
        src_doctoralDegreeFrom.get_versioned_size(NEWT);
    doctoralDegreeFrom.deduplicate();
    ...
    teachingAssistantOf.total_tuples +=
        src_teachingAssistantOf.get_versioned_size(NEWT);
    teachingAssistantOf.deduplicate();
    ...
    undergraduateDegreeFrom.total_tuples +=
        src_undergraduateDegreeFrom.get_versioned_size(NEWT);
    undergraduateDegreeFrom.deduplicate();
    ...
    UndergraduateStudent.total_tuples +=
        src_UndergraduateStudent.get_versioned_size(NEWT);
    UndergraduateStudent.deduplicate();
    ...
    Person.deduplicate();
    Professor.deduplicate();
    Work.deduplicate();
    Organization.deduplicate();
    University.deduplicate();
    degreeFrom.deduplicate();
    Publication.deduplicate();
    member.deduplicate();
    Faculty.deduplicate();
    Student.deduplicate();
    ...
    size_t iteration = 0;
    while (true) {
        RelationVersion incr_ver = DELTA;
        vflog::host_buf_ref_t cached;
        std::cout << "Iteration " << iteration << std::endl;

        // headOf(?X, !Y), Department(!Y) :- Chair(?X) .
        // Person(?X) :- Chair(?X) .
        Person.allocate_newt(Chair.get_versioned_size(incr_ver));
        vflog::column_copy_all(Chair, incr_ver, 0, Person, NEWT, 0, true);
        Person.newt_size += Chair.get_versioned_size(incr_ver);
        Person.total_tuples += Chair.get_versioned_size(incr_ver);
        std::cout << "Person(?X) :- Chair(?X) ." << std::endl;
        // Professor(?X) :- Chair(?X) .
        Professor.allocate_newt(Chair.get_versioned_size(incr_ver));
        vflog::column_copy_all(Chair, incr_ver, 0, Professor, NEWT, 0, true);
        Professor.newt_size += Chair.get_versioned_size(incr_ver);
        Professor.total_tuples += Chair.get_versioned_size(incr_ver);
        std::cout << "Professor(?X) :- Chair(?X) ." << std::endl;

        // Person(?X) :- degreeFrom(?X, ?X1) .
        Person.allocate_newt(degreeFrom.get_versioned_size(incr_ver));
        vflog::column_copy_all(degreeFrom, incr_ver, 0, Person, NEWT, 0, true);
        Person.newt_size += degreeFrom.get_versioned_size(incr_ver);
        Person.total_tuples += degreeFrom.get_versioned_size(incr_ver);
        Person.deduplicate();
        std::cout << "Person(?X) :- degreeFrom(?X, ?X1) ." << std::endl;
        // University(?X1) :- degreeFrom(?X, ?X1) .
        University.allocate_newt(degreeFrom.get_versioned_size(incr_ver));
        vflog::column_copy_all(degreeFrom, incr_ver, 1, University, NEWT, 0,
                               true);
        University.newt_size += degreeFrom.get_versioned_size(incr_ver);
        University.total_tuples += degreeFrom.get_versioned_size(incr_ver);
        // hasAlumnus(?X, ?Y) :- degreeFrom(?Y, ?X) .
        hasAlumnus.allocate_newt(degreeFrom.get_versioned_size(incr_ver));
        vflog::column_copy_all(degreeFrom, incr_ver, 0, hasAlumnus, NEWT, 1,
                               true);
        vflog::column_copy_all(degreeFrom, incr_ver, 1, hasAlumnus, NEWT, 0,
                               true);
        hasAlumnus.newt_size += degreeFrom.get_versioned_size(incr_ver);
        hasAlumnus.total_tuples += degreeFrom.get_versioned_size(incr_ver);

        // Person(?X) :- Employee(?X) .
        Person.allocate_newt(Employee.get_versioned_size(incr_ver));
        vflog::column_copy_all(Employee, incr_ver, 0, Person, NEWT, 0, true);
        Person.newt_size += Employee.get_versioned_size(incr_ver);
        Person.total_tuples += Employee.get_versioned_size(incr_ver);

        // worksFor(?X, !Y), Organization(!Y) :- Employee(?X) .
        // copy the delta id of employee to Organization
        // Organization.allocate_newt(Employee.get_versioned_size(incr_ver));
        // vflog::column_copy_indices(Employee, incr_ver, 0, Organization, NEWT,
        // 0,
        //                            true);
        // Organization.newt_size += Employee.get_versioned_size(incr_ver);
        // Organization.total_tuples += Employee.get_versioned_size(incr_ver);
        // worksFor.allocate_newt(Employee.get_versioned_size(incr_ver));
        // vflog::column_copy_all(Employee, incr_ver, 0, worksFor, NEWT, 0,
        // true); vflog::column_copy_indices(Employee, incr_ver, 0, worksFor,
        // NEWT, 1,
        //                            true);
        // worksFor.newt_size += Employee.get_versioned_size(incr_ver);
        // worksFor.total_tuples += Employee.get_versioned_size(incr_ver);

        // Employee(?X) :- Faculty(?X) .
        Employee.allocate_newt(Faculty.get_versioned_size(incr_ver));
        vflog::column_copy_all(Faculty, incr_ver, 0, Employee, NEWT, 0, true);
        Employee.newt_size += Faculty.get_versioned_size(incr_ver);
        Employee.total_tuples += Faculty.get_versioned_size(incr_ver);

        // Person(?X1) :- hasAlumnus(?X, ?X1) .
        Person.allocate_newt(hasAlumnus.get_versioned_size(incr_ver));
        vflog::column_copy_all(hasAlumnus, incr_ver, 1, Person, NEWT, 0, true);
        Person.newt_size += hasAlumnus.get_versioned_size(incr_ver);
        Person.total_tuples += hasAlumnus.get_versioned_size(incr_ver);
        Person.deduplicate();
        // University(?X) :- hasAlumnus(?X, ?X1) .
        University.allocate_newt(hasAlumnus.get_versioned_size(incr_ver));
        vflog::column_copy_all(hasAlumnus, incr_ver, 0, University, NEWT, 0,
                               true);
        University.newt_size += hasAlumnus.get_versioned_size(incr_ver);
        University.total_tuples += hasAlumnus.get_versioned_size(incr_ver);
        // degreeFrom(?X, ?Y) :- hasAlumnus(?Y, ?X) .
        degreeFrom.allocate_newt(hasAlumnus.get_versioned_size(incr_ver));
        vflog::column_copy_all(hasAlumnus, incr_ver, 1, degreeFrom, NEWT, 0,
                               true);
        vflog::column_copy_all(hasAlumnus, incr_ver, 0, degreeFrom, NEWT, 1,
                               true);
        degreeFrom.newt_size += hasAlumnus.get_versioned_size(incr_ver);
        degreeFrom.total_tuples += hasAlumnus.get_versioned_size(incr_ver);

        // worksFor(?X, ?Y) :- headOf(?X, ?Y) .
        worksFor.allocate_newt(Chair.get_versioned_size(incr_ver));
        // std::cout << "Chair >>>>> " << Chair.get_versioned_size(DELTA) <<
        // std::endl; std::cout << "Chair new >>>>> " <<
        // Chair.get_versioned_size(NEWT) << std::endl; std::cout << "worksFor
        // >>>>> " << worksFor.get_versioned_size(FULL) << std::endl; std::cout
        // << "worksFor size >>>>> " << worksFor.get_versioned_size(NEWT) <<
        // std::endl;
        vflog::column_copy_all(Chair, incr_ver, 0, worksFor, NEWT, 0, true);
        vflog::column_copy_all(Chair, incr_ver, 1, worksFor, NEWT, 1, true);
        worksFor.newt_size += Chair.get_versioned_size(incr_ver);
        worksFor.total_tuples += Chair.get_versioned_size(incr_ver);
        // Organization(?X) :- member(?X, ?X1) .
        Organization.allocate_newt(member.get_versioned_size(incr_ver));
        vflog::column_copy_all(member, incr_ver, 1, Organization, NEWT, 0,
                               true);
        Organization.newt_size += member.get_versioned_size(incr_ver);
        Organization.total_tuples += member.get_versioned_size(incr_ver);
        // Person(?X1) :- member(?X, ?X1) .
        Person.allocate_newt(member.get_versioned_size(incr_ver));
        vflog::column_copy_all(member, incr_ver, 1, Person, NEWT, 0, true);
        Person.newt_size += member.get_versioned_size(incr_ver);
        Person.total_tuples += member.get_versioned_size(incr_ver);
        Person.deduplicate();
        // memberOf(?X, ?Y) :- member(?Y, ?X) .
        memberOf.allocate_newt(member.get_versioned_size(incr_ver));
        vflog::column_copy_all(member, incr_ver, 0, memberOf, NEWT, 1, true);
        vflog::column_copy_all(member, incr_ver, 1, memberOf, NEWT, 0, true);
        memberOf.newt_size += member.get_versioned_size(incr_ver);
        memberOf.total_tuples += member.get_versioned_size(incr_ver);

        // Chair(?X) :- Person(?X), headOf(?X, ?X1), Department(?X1) .
        // TODO:
        cached["Person"] = matched_x_ptr;
        cached["Person"]->resize(Person.get_versioned_size(incr_ver));
        thrust::sequence(cached["Person"]->begin(), cached["Person"]->end());
        vflog::column_join(headOf, FULL, 0, Person, incr_ver, 0, cached,
        ...
        vflog::column_copy(Person, incr_ver, 0, Chair, NEWT, 0,
                           cached["Person"]);
        ...
        cached["Person"]->resize(Person.get_versioned_size(incr_ver));
        thrust::sequence(cached["Person"]->begin(), cached["Person"]->end());
        vflog::column_join(takesCourse, FULL, 0, Person, incr_ver, 0, cached,
                           "Person", matched_x1_ptr);
        ...
        vflog::column_copy(Person, incr_ver, 0, Student, NEWT, 0,
                           cached["Person"]);
        ...
        cached["Person"]->resize(Person.get_versioned_size(incr_ver));
        thrust::sequence(cached["Person"]->begin(), cached["Person"]->end());
        vflog::column_join(teachingAssistantOf, FULL, 0, Person, incr_ver, 0,
        ...
        vflog::column_copy(Person, incr_ver, 0, TeachingAssistant, NEWT, 0,
                           cached["Person"]);
        ...
        cached["Person"]->resize(Person.get_versioned_size(incr_ver));
        thrust::sequence(cached["Person"]->begin(), cached["Person"]->end());
        vflog::column_join(worksFor, FULL, 0, Person, incr_ver, 0, cached,
                           "Person", matched_x1_ptr);
        ...
        vflog::column_copy(Person, incr_ver, 0, Employee, NEWT, 0,
                           cached["Person"]);
        ...
        cached["worksFor"]->resize(worksFor.get_versioned_size(incr_ver));
        thrust::sequence(cached["worksFor"]->begin(),
                         cached["worksFor"]->end());
        vflog::column_join(Person, FULL, 0, worksFor, incr_ver, 0, cached,
                           "worksFor", matched_x1_ptr);
        cached["Person"] = matched_x1_ptr;
        vflog::column_join(Organization, FULL, 0, worksFor, incr_ver, 1, cached,
                           "worksFor", matched_y_ptr, true);
        Employee.allocate_newt(cached["Person"]->size());
        ...
        Faculty.allocate_newt(Professor.get_versioned_size(incr_ver));
        vflog::column_copy_all(Professor, incr_ver, 0, Faculty, NEWT, 0, true);
        Faculty.newt_size += Professor.get_versioned_size(incr_ver);
        Faculty.total_tuples += Professor.get_versioned_size(incr_ver);
        std::cout << "Faculty(?X) :- Professor(?X) ." << std::endl;

        // Organization(?X) :- ResearchGroup(?X) .
        Organization.allocate_newt(ResearchGroup.get_versioned_size(incr_ver));
        vflog::column_copy_all(ResearchGroup, incr_ver, 0, Organization, NEWT,
                               0, true);
        Organization.newt_size += ResearchGroup.get_versioned_size(incr_ver);
        Organization.total_tuples += ResearchGroup.get_versioned_size(incr_ver);
        std::cout << "Organization(?X) :- ResearchGroup(?X) ." << std::endl;
        // Person(?X) :- Student(?X) .
        Person.allocate_newt(Student.get_versioned_size(incr_ver));
        vflog::column_copy_all(Student, incr_ver, 0, Person, NEWT, 0, true);
        Person.newt_size += Student.get_versioned_size(incr_ver);
        Person.total_tuples += Student.get_versioned_size(incr_ver);
        Person.deduplicate();
        std::cout << "Person(?X) :- Student(?X) ." << std::endl;
        // takesCourse(?X, !Y), Course(!Y) :- Student(?X) .
        // Course.allocate_newt(Student.get_versioned_size(incr_ver));
        // vflog::column_copy_indices(Student, incr_ver, 0, Course, NEWT, 0,
        // true); Course.newt_size += Student.get_versioned_size(incr_ver);
        // Course.total_tuples += Student.get_versioned_size(incr_ver);
        // takesCourse.allocate_newt(Student.get_versioned_size(incr_ver));
        // vflog::column_copy_all(Student, incr_ver, 0, takesCourse, NEWT, 0,
        //                        true);
        // vflog::column_copy_indices(Student, incr_ver, 0, takesCourse, NEWT,
        // 1,
        //                            true);
        // takesCourse.newt_size += Student.get_versioned_size(incr_ver);
        // takesCourse.total_tuples += Student.get_versioned_size(incr_ver);

        // worksFor(?X, !Y), ResearchGroup(!Y) :- ResearchAssistant(?X) .
        // ResearchGroup.allocate_newt(
        //     ResearchAssistant.get_versioned_size(incr_ver));
        // vflog::column_copy_all(ResearchAssistant, incr_ver, 1, ResearchGroup,
        //                        NEWT, 0, true);
        // ResearchGroup.newt_size +=
        //     ResearchAssistant.get_versioned_size(incr_ver);
        // ResearchGroup.total_tuples +=
        //     ResearchAssistant.get_versioned_size(incr_ver);
        // worksFor.allocate_newt(ResearchAssistant.get_versioned_size(incr_ver));
        // vflog::column_copy_all(ResearchAssistant, incr_ver, 0, worksFor,
        // NEWT,
        //                        0, true);
        // vflog::column_copy_indices(ResearchAssistant, incr_ver, 0, worksFor,
        //                            NEWT, 1, true);
        // worksFor.newt_size += ResearchAssistant.get_versioned_size(incr_ver);
        // worksFor.total_tuples +=
        // ResearchAssistant.get_versioned_size(incr_ver); Organization(?X) :-
        // subOrganizationOf(?X, ?X1) .
        Organization.allocate_newt(
            subOrganizationOf.get_versioned_size(incr_ver));
        vflog::column_copy_all(subOrganizationOf, incr_ver, 0, Organization,
                               NEWT, 0, true);
        ...
        Organization.allocate_newt(
            subOrganizationOf.get_versioned_size(incr_ver));
        vflog::column_copy_all(subOrganizationOf, incr_ver, 1, Organization,
                               NEWT, 0, true);
        Organization.newt_size +=
            subOrganizationOf.get_versioned_size(incr_ver);
        Organization.total_tuples +=
        ...
        vflog::column_join(src_subOrganizationOf, FULL, 1, subOrganizationOf,
                           incr_ver, 0, cached, "subOrganizationOf",
                           matched_x1_ptr);
        ...
        vflog::column_copy(subOrganizationOf, incr_ver, 1, subOrganizationOf,
        ...
        // Course.allocate_newt(TeachingAssistant.get_versioned_size(incr_ver));
        // vflog::column_copy_indices(TeachingAssistant, incr_ver, 0, Course,
        // NEWT,
        //                            0, true);
        // Course.newt_size += TeachingAssistant.get_versioned_size(incr_ver);
        // Course.total_tuples +=
        // TeachingAssistant.get_versioned_size(incr_ver);
        // teachingAssistantOf.allocate_newt(
        //     TeachingAssistant.get_versioned_size(incr_ver));
        // vflog::column_copy_all(TeachingAssistant, incr_ver, 0,
        //                        teachingAssistantOf, NEWT, 0, true);
        // vflog::column_copy_indices(TeachingAssistant, incr_ver, 0,
        ...
        Course.allocate_newt(teachingAssistantOf.get_versioned_size(incr_ver));
        vflog::column_copy_all(teachingAssistantOf, incr_ver, 1, Course, NEWT,
                               0, true);
        Course.newt_size += teachingAssistantOf.get_versioned_size(incr_ver);
        Course.total_tuples += teachingAssistantOf.get_versioned_size(incr_ver);

        // Organization(?X) :- University(?X) .
        Organization.allocate_newt(University.get_versioned_size(incr_ver));
        vflog::column_copy_all(University, incr_ver, 0, Organization, NEWT, 0,
                               true);
        Organization.newt_size += University.get_versioned_size(incr_ver);
        Organization.total_tuples += University.get_versioned_size(incr_ver);
        // memberOf(?X, ?Y) :- worksFor(?X, ?Y) .
        memberOf.allocate_newt(worksFor.get_versioned_size(incr_ver));
        vflog::column_copy_all(worksFor, incr_ver, 0, memberOf, NEWT, 0, true);
        vflog::column_copy_all(worksFor, incr_ver, 1, memberOf, NEWT, 1, true);
        memberOf.newt_size += worksFor.get_versioned_size(incr_ver);
        memberOf.total_tuples += worksFor.get_versioned_size(incr_ver);

        // deduplicate all recurisve relations and persist
        Chair.deduplicate();
        hasAlumnus.deduplicate();
        Employee.deduplicate();
        worksFor.deduplicate();
        subOrganizationOf.deduplicate();
        teachingAssistantOf.deduplicate();
        Organization.deduplicate();
