...
bool test_conj_head1() {
    ...
    return ram_machine.rels["foobar1"]->tuple_exists(t1, RelationVersion::FULL) &&
           ram_machine.rels["foobar2"]->tuple_exists(t1, RelationVersion::FULL);
}

bool check(std::string testname, bool res) {
    if (res) {
        std::cout << ">>>>>>>>>>>>>>>>>>>>> Test " << testname << " \x1b[1;35;42m passed \033[0m" << std::endl;
    } else {
        std::cout << ">>>>>>>>>>>>>>>>>>>>> Test " << testname << " \x1b[1;31;42m failed \033[0m" << std::endl;
    }
    return res;
}
